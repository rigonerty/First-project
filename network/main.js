...
function commentContain(where, i){
    ...
    if (!i == 0){
        ...
        commentTextArea.style.height = "33px"
        where.prepend(div1)
...
function Chapters(products){
    for (const product of products) {
        if (title.innerHTML == product.name) {
            for(const chapter of product.listChapters){
                ChaptersLoad(chapter)
            }
        }
    }
}
function ChaptersLoad(chapter){
    if (chapter.name != "THIS IS PAGE DON'T EXIST!!"){
